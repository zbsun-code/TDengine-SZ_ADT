...
typedef struct SSQL_S {
    ...
    int8_t rand_ts_range;   // 0: disable random ts range query, 1: enable
    char ts_precision[30];  // "ns" "us" or "ms"
    int64_t ts_start_ns;   // timestamp of the earlist row in the table
    int64_t ts_end_ns;     // timestamp of the latest row in the table
    int64_t ts_intv_ns;    // query timestamp interval
    int64_t* delay_list;
} SSQL;
