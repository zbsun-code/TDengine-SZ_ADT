...
static int getStableInfo(tools_cJSON *dbinfos, int index) {
    ...
    for (int i = 0; i < tools_cJSON_GetArraySize(stables); ++i) {
        ...
        tools_cJSON *rand_seed = tools_cJSON_GetObjectItem(stbInfo, "rand_seed");
        if (tools_cJSON_IsNumber(rand_seed)) {
            g_arguments->rand_seed = (unsigned int) rand_seed->valueint;
        }
            ...
            for (int j = 0; j < specifiedSqlSize; ++j) {
                ...
                if (tools_cJSON_IsObject(sqlObj)) {
                    ...
                    if (tools_cJSON_IsString(sqlStr)) {
                        ...
                        tools_cJSON *ts_precision = tools_cJSON_GetObjectItem(sqlObj, "ts_precision");
                        if (tools_cJSON_IsString(ts_precision)) {
                            strcpy(sql->ts_precision, ts_precision->valuestring);
                        } else {
                            strcpy(sql->ts_precision, "");
                        }
                        tools_cJSON *rand_ts_range = tools_cJSON_GetObjectItem(sqlObj, "rand_ts_range");
                        if (tools_cJSON_IsNumber(rand_ts_range)) {
                            sql->rand_ts_range = rand_ts_range->valueint;
                        } else {
                            sql->rand_ts_range = 0;
                        }
                        tools_cJSON *ts_start_ns = tools_cJSON_GetObjectItem(sqlObj, "ts_start_ns");
                        if (tools_cJSON_IsNumber(ts_start_ns)) {
                            sql->ts_start_ns = ts_start_ns->valueint;
                        } else {
                            sql->ts_start_ns = 0;
                        }
                        tools_cJSON *ts_end_ns = tools_cJSON_GetObjectItem(sqlObj, "ts_end_ns");
                        if (tools_cJSON_IsNumber(ts_end_ns)) {
                            sql->ts_end_ns = ts_end_ns->valueint;
                        } else {
                            sql->ts_end_ns = 0;
                        }
                        tools_cJSON *ts_intv_ns = tools_cJSON_GetObjectItem(sqlObj, "ts_intv_ns");
                        if (tools_cJSON_IsNumber(ts_intv_ns)) {
                            sql->ts_intv_ns = ts_intv_ns->valueint;
                        } else {
                            sql->ts_intv_ns = 0;
                        }
                    } else {
