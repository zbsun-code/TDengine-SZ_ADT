...
static void *specifiedTableQuery(void *sarg) {
    ...
    while (index < queryTimes) {
        if (g_queryInfo.specifiedQueryInfo.queryInterval &&
            ...
            //                      (et - st)));  // ms
            usleep((int32_t)(g_queryInfo.specifiedQueryInfo.queryInterval -
                                 (et - st)));  // ms
        }
        if (g_queryInfo.reset_query_cache) {
            queryDbExec(pThreadInfo->conn, "reset query cache");
        ...
        replaceTblName(sql->command, sqlstr,
                    rand() % g_queryInfo.specifiedQueryInfo.tableCount);
        if (sql->rand_ts_range) {
        // TODO: now we got sql->(ts_start_ns, ts_end_ns, ts_intv_ns), add random ts range after the query str.
            int64_t max_tsrange_start = sql->ts_end_ns - sql->ts_intv_ns;
            int64_t rand_range = max_tsrange_start - sql->ts_start_ns;
            int64_t cur_start = (((int64_t)rand() << 31) + rand()) % rand_range + sql->ts_start_ns;  // 62 bits random integer (enough for ns until about 2116-02-21 07:53:38)
            char ts_case[100] = {0};
            if (strcmp(sql->ts_precision, "ns") == 0) {
                sprintf(ts_case, " where ts >= %ld and ts <= %ld", cur_start, cur_start + sql->ts_intv_ns);
            } else if (strcmp(sql->ts_precision, "us") == 0) {
                sprintf(ts_case, " where ts >= %ld and ts <= %ld", cur_start / 1000, (cur_start + sql->ts_intv_ns) / 1000);
            } else if (strcmp(sql->ts_precision, "ms") == 0) {
                sprintf(ts_case, " where ts >= %ld and ts <= %ld", cur_start / 1000000, (cur_start + sql->ts_intv_ns) / 1000000);
            } else {
                printf("error: ts_precision is \"%s\"", sql->ts_precision);
            }
            strcat(sqlstr, ts_case);
            // fprintf(stderr, "%s\n", sqlstr);
        }
        st = toolsGetTimestampUs();
...
static int multi_thread_specified_table_query(uint16_t iface, char* dbName) {
    ...
    tmfree(g_queryInfo.specifiedQueryInfo.tableNameList);
...
int queryTestProcess() {
    ...
    g_queryInfo.specifiedQueryInfo.tableNameList = getAllTables(conn->taos, g_queryInfo.dbName, &g_queryInfo.specifiedQueryInfo.tableCount);
    if (!g_queryInfo.specifiedQueryInfo.tableNameList) {
        tmfree(g_queryInfo.specifiedQueryInfo.tableNameList);
