...
TAOS_ROW taos_fetch_row(TAOS_RES *res) {
  ...
  // static time_t et = 0, st = 0;
  if (pRes->row >= pRes->numOfRows && needToFetchNewBlock(pSql)) {
    // et = taosGetTimestamp(TSDB_TIME_PRECISION_MICRO);
    // printf("Fetch loop elapsed time: %ld us\n", et - st);
    taos_fetch_rows_a(res, waitForRetrieveRsp, pSql->pTscObj);
    tsem_wait(&pSql->rspSem);
    // st = taosGetTimestamp(TSDB_TIME_PRECISION_MICRO);
    // printf("Block transfer elapsed time: %ld us\n", st - et);
  }
