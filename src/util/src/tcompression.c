...
int tsCompressInit(){
  ...
  tdszInit(fPrecision, dPrecision, maxRange, curRange, entropy_type, huffman_force, Compressor);
  if(lossyFloat)
...
int tsCompressFloatLossyImp(const char * input, const int nelements, char *const output){
  ...
  if (compressedSize == 0 || (huffman_force == 0 && compressedSize >= nelements*sizeof(float))){//zy
